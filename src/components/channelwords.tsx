...
interface KeywordsProps {
  ...
  saveKeywords: (payload: { channels: TagInfo[] }) => void;
}

const Keywords = (props: KeywordsProps) => {
  const { channels, toggleTips, saveKeywords } = props;
  ...
  const handleClose = (removedTag: string) => {
    const newTags = channels.filter((tag) => tag.value !== removedTag);
    saveKeywords({ channels: newTags });
  };
  ...
  const handleInputConfirm = () => {
    const existTag = channels.find((tag) => tag.value === inputValue);
    if (inputValue && existTag) {
      toggleTips('Keyword already exists', 'error');
      return;
    }
    if (inputValue && !existTag) {
      saveKeywords({ channels: [...channels, { color: getColor(), value: inputValue}] });
